...
import { galleryForVariant, selectGalleryCover, selectVariantDisplayMedia } from '../../core/product-media.util';
import {
  attributeKeys,
  attributeLabel,
  colorSwatch,
          ...
          <div>
            ...
            >
              @if (selectedMedia(); as media) {
                <img class="h-full w-full object-cover transition duration-300 group-hover:scale-[1.015]" [src]="media.url" [alt]="media.alt" decoding="async" />
                <span class="absolute bottom-3 right-3 inline-flex size-10 items-center justify-center rounded-lg bg-black/55 text-white shadow-sm backdrop-blur transition duration-300 group-hover:scale-105 group-focus-visible:scale-105" aria-hidden="true">
          ...
          <div class="lg:pt-8">
            ...
            @if (usesStructuredAttributes(item)) {
              @for (key of structuredAttributeKeys(item); track key) {
                <fieldset class="mt-8">
                  <legend class="text-sm font-extrabold uppercase tracking-wide text-[var(--color-text-muted)]">{{ attributeLabel(key) }}</legend>
                  @if (attributeOptions(item, key).length) {
                      ...
                      @for (value of attributeOptions(item, key); track value) {
                        ...
                        >
                          @if (key === 'color' && colorSwatch(value); as swatch) {
                            <span class="size-3 rounded-full border border-[var(--color-text-muted)] shadow-[0_0_0_2px_var(--color-card)]" [style.background-color]="swatch" aria-hidden="true"></span>
                          }
                          {{ value }}
                        </button>
                      }
                    ...
                    <p class="mt-3 text-sm font-bold text-[var(--color-text-muted)]">{{ emptyAttributeMessage(key) }}</p>
                  }
                </fieldset>
              }
            } @else {
            ...
            @if (selectedVariant(); as variant) {
              ...
              <p class="mt-5 font-bold text-[var(--color-text-muted)]">
                {{ usesStructuredAttributes(item) ? 'Elegí las opciones para continuar.' : 'Sin stock.' }}
              </p>
              <button class="button-primary mt-6 min-h-12 rounded-full px-7 font-extrabold opacity-50" type="button" disabled>
                Agregar al carrito
              </button>
            }
...
export class ProductDetailPageComponent implements OnInit {
  ...
  attributeOptions(product: PublicProduct, key: string): string[] {
    ...
    return [...new Set(
      product.variants
        .filter((variant) => previousKeys.every((previous) => variantAttribute(variant, previous) === this.selectedAttributes()[previous]))
  ...
  isAttributeOptionDisabled(product: PublicProduct, key: string, value: string): boolean {
    ...
    const matching = product.variants.filter(
      (variant) =>
        ...
        previousKeys.every((previous) => variantAttribute(variant, previous) === this.selectedAttributes()[previous]),
    );
  ...
  attributeButtonClass(product: PublicProduct, key: string, value: string): string {
    ...
    const base = 'min-h-11 border px-4 text-sm font-extrabold transition disabled:cursor-not-allowed disabled:opacity-40 ' + shape;
    return selected
      ? base + ' border-[var(--color-accent)] bg-[var(--color-accent)] text-white'
      : base + ' border-[var(--color-border)] bg-[var(--color-card)] hover:border-[var(--color-accent)]';
  }
  ...
  galleryMedia(product: PublicProduct): PublicProductMedia[] {
    const variant = this.selectedVariant() ?? this.variantForAttributes(product, this.selectedAttributes(), true);
  ...
  priceLabel(product: PublicProduct): string {
    ...
    return prices.every((price) => price === min) ? this.money(min) : 'Desde ' + this.money(min);
  }
  ...
  private initializeSelection(product: PublicProduct): void {
    ...
    const firstValue = firstKey
      ? this.attributeOptions(product, firstKey).find((value) => !this.isAttributeOptionDisabled(product, firstKey, value))
      : null;
